...
void parkinglot(Car *car)
{
    while (true) {
        ...
        stdio_mutex.lock();
        displayColor(spot, LCD_COLOR_BLACK);
        stdio_mutex.unlock();
...
int main(void)
{
    ...
    car1.start(callback(threading,&c1));
    car2.start(callback(threading,&c2));
    car3.start(callback(threading,&c3));
    car4.start(callback(threading,&c4));
    car5.start(callback(threading,&c5));
}